...
export default function ResultDashboard({ data, cac }) {
    if (!data?.length) return null;

    function getSummary(data, cac) {
        ...
        const avgLTV = data.reduce((sum, d) => sum + d.ltv, 0) / data.length;
        const ltvToCacRatio = cac > 0 ? (avgLTV / cac).toFixed(2) : 'N/A';
    ...
    const summary = getSummary(data, cac);
    ...
    return (
        ...
        <h2 className="text-xl font-semibold">Simulation Results</h2>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-gray-50 p-4 rounded shadow">
                ...
                <div>
                    <div className="text-sm text-gray-600">LTV to CAC Ratio</div>
                    <div className="text-lg font-bold">{summary.ltvToCacRatio}</div>
                </div>
